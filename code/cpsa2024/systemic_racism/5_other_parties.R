...
graph_data <- potgrowth::dynamic_potgrowth_data(
  data = survey_data,
  parties = c("PLQ", "PCQ"),
  issues = "iss_newleft_wokeSysRaci",
) %>% 
...
graph_data %>%
  ...
  xlab("\nThere is systemic racism in Quebec") +
  ylab("Potential for Growth\n(predicted RCI of non-voters)")

ggsave("_SharedFolder_memoire-pot-growth/graphs/cpsa2024/dynamic_potgrowth/systemic_racism/5_other_parties.png",
