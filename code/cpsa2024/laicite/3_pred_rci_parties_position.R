...
graph_data <- potgrowth::dynamic_potgrowth_data(
  data = survey_data,
  parties = c("PQ", "CAQ", "QS"),
  issues = "iss_laic_relSignsTeachersNo",
) %>% 
...
graph_data %>%
  ...
  geom_point(aes(alpha = is_party_position, fill = party),
             position = position_dodge(width = 0.6),
             size = 15, shape = 23, show.legend = FALSE) +
  ...
  xlab("\nTeachers should be allowed to wear\nvisible religious symbols at work.") +
  ylab("Potential for Growth\n(predicted RCI of non-voters)")

ggsave("_SharedFolder_memoire-pot-growth/graphs/cpsa2024/dynamic_potgrowth/laicite/3_pred_rci_parties_position.png",
